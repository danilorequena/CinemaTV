...
protocol PersonServiceProtocol: AnyObject {
    func fetchPerson(from endpoint: MoviesEndpoint, completion: @escaping (Result<Person, RequestError>) -> Void)
    func fetchCreditsByPerson(from endpoint: MoviesEndpoint, completion: @escaping (Result<CreditsByPerson, RequestError>) -> Void)
}

final class PersonStore: PersonServiceProtocol {
